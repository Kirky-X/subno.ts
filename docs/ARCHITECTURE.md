<div align="center">

# 架构设计文档

### SecureNotify 系统架构详解

[🏠 首页](../README.md) • [📖 用户指南](USER_GUIDE.md) • [📘 API 参考](API_REFERENCE.md)

---

</div>

## 目录

- [系统概览](#系统概览)
- [系统架构](#系统架构)
- [核心组件](#核心组件)
- [数据流程](#数据流程)
- [安全性设计](#安全性设计)
- [技术决策](#技术决策)
- [性能考虑](#性能考虑)
- [可扩展性](#可扩展性)
- [监控与运维](#监控与运维)

---

## 系统概览

### 产品定位

**SecureNotify** 是一个专注于**端到端加密**的推送通知服务，旨在为应用程序提供安全、实时、可靠的消息分发能力。

### 设计目标

| 目标 | 描述 |
|------|------|
| **安全性** | 端到端加密，密钥管理，审计日志 |
| **实时性** | 基于 SSE 的实时消息推送 |
| **可靠性** | 消息持久化，重连机制，优先级队列 |
| **可扩展性** | 水平扩展，支持高并发 |
| **可观测性** | 完整的日志、监控、审计追踪 |

### 核心指标

| 指标 | 目标值 |
|------|--------|
| 消息延迟 | < 100ms (端到端) |
| 连接稳定性 | 99.9% 可用性 |
| 消息持久化 | 12 小时 |
| 并发连接数 | 10,000+ SSE 连接 |

---

## 系统架构

### 整体架构图

```
┌─────────────────────────────────────────────────────────────────────────┐
│                           SecureNotify 架构                              │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  ┌──────────────┐     ┌──────────────┐     ┌──────────────┐           │
│  │   Client 1   │     │   Client 2   │     │   Client N   │           │
│  │  (Browser)   │     │  (Mobile)    │     │    (Other)   │           │
│  └──────┬───────┘     └──────┬───────┘     └──────┬───────┘           │
│         │                    │                    │                     │
│         └────────────────────┼────────────────────┘                     │
│                              │                                          │
│                              ▼                                          │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │                      Next.js API Gateway                          │  │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐               │  │
│  │  │  CORS/Security│  │   Rate      │  │   Auth      │               │  │
│  │  │   Middleware │  │   Limiting  │  │   Middleware│               │  │
│  │  └─────────────┘  └─────────────┘  └─────────────┘               │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│                              │                                          │
│         ┌────────────────────┼────────────────────┐                    │
│         │                    │                    │                    │
│         ▼                    ▼                    ▼                    │
│  ┌──────────────┐     ┌──────────────┐     ┌──────────────┐           │
│  │   /api/register│   │   /api/channels│   │   /api/publish│           │
│  │  (公钥注册)    │   │   (频道管理)   │   │   (消息发布)   │           │
│  └──────────────┘     └──────────────┘     └──────────────┘           │
│         │                    │                    │                     │
│         ▼                    ▼                    ▼                     │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │                     业务逻辑层 (Services)                          │  │
│  │  ┌────────────┐ ┌────────────┐ ┌────────────┐ ┌────────────┐     │  │
│  │  │Encryption  │ │  Channel   │ │  Message   │ │   Audit    │     │  │
│  │  │   Service  │ │  Service   │ │  Service   │ │  Service   │     │  │
│  │  └────────────┘ └────────────┘ └────────────┘ └────────────┘     │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│                              │                                          │
│         ┌────────────────────┼────────────────────┐                    │
│         │                    │                    │                    │
│         ▼                    ▼                    ▼                    │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │                     数据访问层 (Repositories)                      │  │
│  │  ┌────────────┐ ┌────────────┐ ┌────────────┐ ┌────────────┐     │  │
│  │  │ PublicKey  │ │  Channel   │ │  Message   │ │  ApiKey    │     │  │
│  │  │ Repository │ │ Repository │ │ Repository │ │ Repository │     │  │
│  │  └────────────┘ └────────────┘ └────────────┘ └────────────┘     │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│                              │                                          │
│         ┌────────────────────┼────────────────────┐                    │
│         ▼                    ▼                    ▼                    │
│  ┌──────────────┐     ┌──────────────┐     ┌──────────────┐           │
│  │  PostgreSQL  │     │     Redis    │     │  Audit Logs  │           │
│  │  (主数据库)   │     │  (缓存/队列)  │     │  (存储)       │           │
│  └──────────────┘     └──────────────┘     └──────────────┘           │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

### 分层架构

```
┌─────────────────────────────────────────────────────────┐
│                   API Gateway Layer                      │
│         (Next.js App Router + Middleware)               │
├─────────────────────────────────────────────────────────┤
│                   Presentation Layer                     │
│              (API Routes / Request Handlers)             │
├─────────────────────────────────────────────────────────┤
│                  Business Logic Layer                    │
│              (Services / 业务逻辑)                       │
├─────────────────────────────────────────────────────────┤
│                 Data Access Layer                        │
│            (Repositories / 数据访问对象)                 │
├─────────────────────────────────────────────────────────┤
│                     Data Layer                           │
│         (PostgreSQL + Redis + File Storage)             │
└─────────────────────────────────────────────────────────┘
```

---

## 核心组件

### 1. API Gateway

**技术栈**: Next.js 16 + TypeScript

**职责**:
- HTTP 请求路由
- 请求/响应处理
- 中间件集成 (CORS, Security Headers)
- 认证/授权

**关键配置**:
```typescript
// middleware.ts
export function middleware(request: NextRequest) {
  // CORS 配置
  // 安全响应头
  // 请求验证
}
```

### 2. 加密服务 (Encryption Service)

**位置**: `src/lib/services/encryption/`

#### 2.1 RSA 加密服务

```typescript
// rsa.service.ts
class RsaService {
  // 生成 RSA 密钥对
  generateKey(keySize: number = 4096): KeyPair
  
  // 公钥加密
  encrypt(plaintext: string, publicKey: string): string
  
  // 私钥解密
  decrypt(ciphertext: string, privateKey: string): string
  
  // 数字签名
  sign(message: string, privateKey: string): string
  
  // 签名验证
  verify(signature: string, message: string, publicKey: string): boolean
}
```

**支持的算法**:
- RSA-2048 (默认)
- RSA-4096 (增强安全)
- ECC-SECP256K1 (椭圆曲线)

#### 2.2 AES 加密服务

```typescript
// aes.service.ts
class AesService {
  // 加密 (AES-256-GCM)
  encrypt(plaintext: string, key?: Buffer): EncryptResult
  
  // 解密
  decrypt(ciphertext: string, iv: string, tag: string, key: Buffer): string
  
  // 生成密钥
  generateKey(): Buffer
}
```

**规格**:
- 算法: AES-256-GCM
- 密钥长度: 32 字节 (256 位)
- IV 长度: 16 字节 (128 位)
- 认证标签: 16 字节

#### 2.3 混合加密服务

```typescript
// hybrid.service.ts
class HybridService {
  // 加密大消息
  encrypt(
    message: string,
    recipientPublicKey: string
  ): HybridEncryptResult
  
  // 解密消息
  decrypt(
    encryptedMessage: HybridEncryptResult,
    recipientPrivateKey: string
  ): string
}
```

**加密流程**:
```
1. 生成随机 AES-256 会话密钥
2. 用接收者公钥加密 AES 密钥 (RSA-OAEP)
3. 用 AES-GCM 加密消息内容
4. 返回: 加密的 AES 密钥 + IV + 标签 + 密文
```

### 3. 消息服务

**职责**:
- 消息发布
- 消息订阅 (SSE)
- 消息队列管理
- 优先级处理

**消息优先级**:
```typescript
enum MessagePriority {
  CRITICAL = 100,  // 立即处理
  HIGH = 75,       // 高优先级
  NORMAL = 50,     // 默认
  LOW = 25,        // 低优先级
  BULK = 0         // 批量
}
```

### 4. 频道服务

**职责**:
- 频道 CRUD 操作
- 频道类型管理
- 过期清理
- 自动创建临时频道

**频道类型**:
| 类型 | 前缀 | 加密 | TTL |
|------|------|------|-----|
| 公开 | `pub_` | ❌ | 可配置 |
| 加密 | `enc_` | ✅ | 可配置 |
| 临时 | `tmp_` | ❌/✅ | 30 分钟 |

### 5. 密钥管理服务

**职责**:
- 公钥注册/查询/撤销
- API 密钥管理
- 密钥缓存
- 密钥过期处理

**密钥缓存策略** (Cache-Aside 模式):
```typescript
// key-cache.service.ts
class KeyCacheService {
  async get(publicKeyId: string): Promise<PublicKey | null> {
    // 1. 先查 Redis 缓存
    const cached = await redis.get(`key:${publicKeyId}`);
    if (cached) return cached;
    
    // 2. 缓存未命中，查数据库
    const key = await db.publicKeys.findById(publicKeyId);
    if (key) {
      // 3. 写入缓存 (TTL: 7 天)
      await redis.setex(`key:${publicKeyId}`, 604800, key);
    }
    return key;
  }
}
```

### 6. 审计日志服务

**职责**:
- 操作记录
- 日志存储
- 日志查询
- 日志清理

**审计日志结构**:
```typescript
interface AuditLog {
  id: UUID;
  createdAt: Date;
  action: string;        // key_register, message_publish, etc.
  channelId?: string;
  keyId?: string;
  messageId?: string;
  userId?: string;
  ip?: string;
  userAgent?: string;
  success: boolean;
  error?: string;
  metadata?: Record<string, unknown>;
}
```

---

## 数据流程

### 消息发布流程

```
┌─────────────────────────────────────────────────────────────────┐
│                     消息发布流程                                  │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Client                                                          │
│    │                                                            │
│    ▼                                                            │
│  POST /api/publish                                               │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  1. 请求验证 (Zod Schema Validation)                       │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  2. 速率限制检查 (Redis Counter)                            │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    ├─ 超过限制 ──→ 返回 429 Too Many Requests                  │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  3. 认证检查 (X-API-Key Header)                             │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  4. 频道验证/创建                                           │   │
│  │     - 检查频道是否存在                                      │   │
│  │     - 如果 autoCreate，创建临时频道                         │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  5. 消息加密 (如果 encrypted: true)                         │   │
│  │     - 使用混合加密 (RSA + AES-256-GCM)                     │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  6. 消息存储                                               │   │
│  │     - 写入 Redis Sorted Set (按优先级排序)                  │   │
│  │     - 发布到 Redis Pub/Sub                                 │   │
│  │     - 持久化到 PostgreSQL (如果 cache: true)               │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  7. 审计日志记录                                           │   │
│  │     - 记录操作类型、时间、结果                              │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    ▼                                                            │
│  返回 201 Created ({ messageId, timestamp })                    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 实时订阅流程

```
┌─────────────────────────────────────────────────────────────────┐
│                     实时订阅流程 (SSE)                            │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Client                                                          │
│    │                                                            │
│    ▼                                                            │
│  GET /api/subscribe?channel=my-channel                           │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  1. 频道验证                                               │   │
│  │     - 检查频道是否存在                                      │   │
│  │     - 检查频道是否过期                                      │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  2. 建立 SSE 连接                                          │   │
│  │     - 设置 Content-Type: text/event-stream                 │   │
│  │     - 设置 Cache-Control: no-cache                          │   │
│  │     - 设置 Connection: keep-alive                           │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  3. 发送连接确认                                           │   │
│  │     event: connected                                       │   │
│  │     data: {"channel": "my-channel", "status": "active"}    │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  4. 订阅 Redis Pub/Sub                                     │   │
│  │     SUBSCRIBE channel:my-channel                           │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    │    ┌───────────────────────────────────────────────────┐  │
│    │    │                  消息循环                          │  │
│    │    │                                                   │  │
│    │    │  ┌─────────────────────────────────────────────┐  │  │
│    │    │  │ 等待新消息 (BLPOP)                            │  │  │
│    │    │  └─────────────────────────────────────────────┘  │  │
│    │    │                    │                             │  │
│    │    │                    ▼                             │  │
│    │    │  ┌─────────────────────────────────────────────┐  │  │
│    │    │  │ 发送 SSE 消息                                 │  │  │
│    │    │  │ event: message                               │  │  │
│    │    │  │ id: msg_xxx                                  │  │  │
│    │    │  │ data: {"id": "...", "message": "..."}        │  │  │
│    │    │  └─────────────────────────────────────────────┘  │  │
│    │    │                    │                             │  │
│    │    └────────────────────┼─────────────────────────────┘  │
│    │                         │                                  │
│    │    定时 (每 30 秒)       │ 新消息                          │
│    │                         │                                  │
│    │    ┌────────────────────┴──────────────┐                 │
│    │    │                                    │                 │
│    │    ▼                                    ▼                 │
│    │  发送 Keepalive                        处理消息            │
│    │  : keepalive                                                   │
│    │                                                               │
│    │  客户端断开连接                                                 │
│    │    │                                                         │
│    │    ▼                                                         │
│    │  清理资源 (关闭 Redis 连接)                                      │
│    │                                                               │
└─────────────────────────────────────────────────────────────────┘
```

### 密钥注册流程

```
┌─────────────────────────────────────────────────────────────────┐
│                     公钥注册流程                                  │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Client                                                          │
│    │                                                            │
│    ▼                                                            │
│  POST /api/register                                               │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  1. 请求验证                                               │   │
│  │     - 验证 publicKey 格式 (PEM)                            │   │
│  │     - 验证 algorithm 参数                                   │   │
│  │     - 验证 expiresIn 范围                                   │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  2. 生成频道 ID                                             │   │
│  │     - 格式: enc_{hash}                                     │   │
│  │     - 确保唯一性                                            │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  3. 存储公钥                                               │   │
│  │     - 写入 PostgreSQL public_keys 表                       │   │
│  │     - 创建频道记录 (channels 表)                           │   │
│  │     - 设置过期时间                                         │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  4. 初始化缓存                                             │   │
│  │     - 写入 Redis 缓存 (TTL: 7 天)                          │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    ▼                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  5. 审计记录                                               │   │
│  │     - 记录 key_register 操作                               │   │
│  └──────────────────────────────────────────────────────────┘   │
│    │                                                            │
│    ▼                                                            │
│  返回 201 Created                                               │
│  {                                                              │
│    "success": true,                                             │
│    "data": {                                                    │
│      "channelId": "enc_xxx",                                    │
│      "publicKeyId": "uuid",                                     │
│      "algorithm": "RSA-4096",                                   │
│      "expiresAt": "...",                                        │
│      "expiresIn": 604800                                        │
│    }                                                            │
│  }                                                              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 安全性设计

### 加密体系

```
┌─────────────────────────────────────────────────────────────────┐
│                      加密层次结构                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Layer 1: 传输加密                                               │
│  ─────────────────                                              │
│  • HTTPS/TLS 1.3                                                │
│  • 证书验证                                                      │
│  • 安全的 TLS 配置                                               │
│                                                                 │
│  Layer 2: 应用层加密                                             │
│  ─────────────────────                                          │
│  • 端到端加密 (E2EE)                                             │
│  • 混合加密 (RSA + AES-256-GCM)                                 │
│  • 数字签名                                                      │
│                                                                 │
│  Layer 3: 数据存储加密                                           │
│  ─────────────────────                                          │
│  • 敏感数据加密存储                                               │
│  • 密钥哈希存储                                                   │
│  • 内存中的密钥保护                                               │
│                                                                 │
│  Layer 4: 访问控制                                               │
│  ─────────────────                                              │
│  • API 密钥认证                                                  │
│  • 权限控制                                                      │
│  • IP 白名单                                                    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 认证机制

```typescript
// 认证流程
async function authenticate(request: Request): Promise<AuthResult> {
  // 1. 检查 X-API-Key
  const apiKey = request.headers.get('X-API-Key');
  if (apiKey) {
    return validateApiKey(apiKey);
  }
  
  // 2. 检查 X-Admin-Key
  const adminKey = request.headers.get('X-Admin-Key');
  if (adminKey) {
    return validateAdminKey(adminKey);
  }
  
  // 3. 检查 Cron Secret
  const cronSecret = request.headers.get('X-Cron-Secret');
  if (cronSecret) {
    return validateCronSecret(cronSecret);
  }
  
  // 4. 无认证的公开端点
  return { authenticated: false, userId: null };
}
```

### 速率限制

```typescript
// 多层速率限制
class RateLimiter {
  // 基于 IP 的严格限制
  async checkIpLimit(ip: string, limit: number, window: number): Promise<boolean>
  
  // 基于 User-Agent 的宽松限制
  async checkUaLimit(uaHash: string, limit: number, window: number): Promise<boolean>
  
  // 组合检查
  async checkPublishLimit(request: Request): Promise<boolean> {
    const ip = this.getClientIp(request);
    const uaHash = this.hashUserAgent(request.headers.get('user-agent'));
    
    return (
      await this.checkIpLimit(ip, 10, 60) &&  // 严格: 10次/分钟
      await this.checkUaLimit(uaHash, 20, 60) // 宽松: 20次/分钟
    );
  }
}
```

### 安全响应头

```typescript
// middleware.ts - 安全响应头配置
const securityHeaders = {
  'X-DNS-Prefetch-Control': 'off',
  'Strict-Transport-Security': 'max-age=63072000; includeSubDomains; preload',
  'X-Content-Type-Options': 'nosniff',
  'X-Frame-Options': 'DENY',
  'X-XSS-Protection': '1; mode=block',
  'Referrer-Policy': 'origin-when-cross-origin',
  'Content-Security-Policy': "default-src 'self'"
};
```

### 审计日志

```typescript
// 审计日志记录
await auditService.log({
  action: 'message_publish',
  channelId: 'my-channel',
  userId: 'user-123',
  ip: request.ip,
  userAgent: request.headers.get('user-agent'),
  success: true,
  metadata: { messageId: 'msg_xxx', priority: 'normal' }
});
```

---

## 技术决策

### 数据库选择

| 组件 | 选择 | 原因 |
|------|------|------|
| **主数据库** | PostgreSQL 14+ | 可靠性、ACID 事务、JSON 支持 |
| **缓存/消息** | Redis 7+ | 高性能、Pub/Sub、Sorted Sets |
| **ORM** | Drizzle ORM | 轻量级、类型安全、零依赖 |

### 消息队列设计

```typescript
// 优先级队列实现
// 使用 Redis Sorted Set，score 为优先级值
await redis.zadd(
  `channel:${channelId}:messages`,
  {
    [messageId]: priority,  // CRITICAL=100, HIGH=75, NORMAL=50, LOW=25, BULK=0
    [data]: messageData
  }
);

// 按优先级取消息
await redis.zrevrange(
  `channel:${channelId}:messages`,
  0,
  count - 1  // 取最高优先级的消息
);
```

### 实时通信

```typescript
// Server-Sent Events (SSE) 优于 WebSocket 的原因
const sseAdvantages = [
  '自动重连支持',
  '简单协议 (HTTP)',
  '防火墙友好',
  '单向通信 (更适合推送)',
  '浏览器原生支持'
];

// 备选方案：Redis Pub/Sub
// 用于多实例间的消息分发
await redis.subscribe(`channel:${channelId}`, (message) => {
  // 广播到所有订阅者
});
```

### 密钥存储策略

```
┌─────────────────────────────────────────────────────────────────┐
│                    密钥存储层次                                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  热数据 (频繁访问)                                               │
│  ─────────────────                                              │
│  Redis Cache (7 天 TTL)                                         │
│  • 公钥查询                                                     │
│  • API 密钥验证                                                 │
│                                                                 │
│  温数据 (持久存储)                                               │
│  ─────────────────                                              │
│  PostgreSQL                                                     │
│  • 公钥 (public_keys 表)                                        │
│  • API 密钥 (api_keys 表 - 哈希存储)                            │
│  • 频道元数据                                                   │
│                                                                 │
│  冷数据 (归档)                                                  │
│  ────────────                                                  │
│  审计日志 (90 天保留)                                           │
│  过期消息                                                       │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 性能考虑

### 性能指标

| 指标 | 目标 | 当前实现 |
|------|------|----------|
| API 响应时间 | P95 < 100ms | ~50ms |
| 消息推送延迟 | < 50ms | ~20ms |
| SSE 连接建立 | < 200ms | ~100ms |
| 并发连接数 | 10,000+ | 受限于单实例 |
| 消息吞吐量 | 1,000 msg/s | 可水平扩展 |

### 优化策略

```typescript
// 1. 连接池配置
const poolConfig = {
  min: 5,
  max: 20,
  idleTimeout: 30000,
  connectionTimeoutMillis: 5000
};

// 2. Redis 连接复用
const redis = createRedisClient({
  maxRetriesPerRequest: 3,
  enableReadyCheck: true,
  lazyConnect: true
});

// 3. 批处理优化
async function batchProcess(items: Item[]) {
  const batchSize = 100;
  const batches = chunk(items, batchSize);
  
  for (const batch of batches) {
    await Promise.all(batch.map(item => process(item)));
  }
}

// 4. 缓存策略
// 公钥缓存 7 天
// 频道信息缓存 1 小时
// API 密钥验证结果缓存 5 分钟
```

### 瓶颈分析

```
┌─────────────────────────────────────────────────────────────────┐
│                      性能瓶颈分析                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  数据库查询                                                      │
│  ├─ 公钥查询: 读多写少，主要读 Redis 缓存                         │
│  ├─ 频道创建: 写操作，需要唯一性检查                              │
│  └─ 消息存储: 批量写入，优化批次大小                              │
│                                                                 │
│  Redis 操作                                                      │
│  ├─ Pub/Sub: 高频，但延迟低                                      │
│  ├─ Sorted Set: 优先级排序，O(log N)                             │
│  └─ 缓存失效: 需要注意缓存击穿                                    │
│                                                                 │
│  网络传输                                                        │
│  ├─ SSE 连接: 长连接，资源占用                                   │
│  ├─ 消息加密: CPU 密集型，可考虑硬件加速                          │
│  └─ TLS 开销: 使用 TLS 1.3 优化                                  │
│                                                                 │
│  内存使用                                                        │
│  ├─ SSE 连接: 每个连接约 1KB                                     │
│  ├─ 消息队列: 需要限制队列长度                                    │
│  └─ 缓存: 配置合理的 TTL 和最大内存                              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 可扩展性

### 水平扩展架构

```
                           ┌─────────────────┐
                           │   Load Balancer │
                           │   (Nginx/Cloud) │
                           └────────┬────────┘
                                    │
              ┌─────────────────────┼─────────────────────┐
              │                     │                     │
              ▼                     ▼                     ▼
     ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
     │  Next.js Node 1 │  │  Next.js Node 2 │  │  Next.js Node N │
     │   (Instance)    │  │   (Instance)    │  │   (Instance)    │
     └────────┬────────┘  └────────┬────────┘  └────────┬────────┘
              │                     │                     │
              └─────────────────────┼─────────────────────┘
                                    │
                                    ▼
              ┌─────────────────────┼─────────────────────┐
              │                     │                     │
              ▼                     ▼                     ▼
     ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
     │   PostgreSQL    │  │   PostgreSQL    │  │   PostgreSQL    │
     │   (Primary)     │  │    (Replica)    │  │    (Replica)    │
     └─────────────────┘  └─────────────────┘  └─────────────────┘
                                    │
                                    ▼
     ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
     │   Redis Master  │  │   Redis Slave   │  │   Redis Slave   │
     │                 │  │  (Read Replica) │  │  (Read Replica) │
     └─────────────────┘  └─────────────────┘  └─────────────────┘
```

### 扩展策略

| 扩展维度 | 策略 |
|----------|------|
| **计算层** | 无状态 Next.js 实例，Kubernetes HPA 自动扩缩容 |
| **数据库层** | PostgreSQL 主从复制 + 连接池分片 |
| **缓存层** | Redis 集群 + Sentinel 高可用 |
| **消息层** | Redis Pub/Sub + 多消费者组 |
| **CDN** | 静态资源通过 CDN 分发 |

### 分片策略

```typescript
// 基于频道 ID 的分片
function getShardId(channelId: string): number {
  const hash = crypto.createHash('md5').update(channelId).digest('hex');
  const shardNum = parseInt(hash.substring(0, 2), 16) % SHARD_COUNT;
  return shardNum;
}

// 分片配置
const SHARD_CONFIG = {
  count: 4,
  replicaCount: 2,
  shardingKey: 'channelId'
};
```

---

## 监控与运维

### 监控指标

```typescript
// 关键指标定义
const metrics = {
  // API 指标
  api: {
    requests: 'counter',      // 请求总数
    latency: 'histogram',      // 响应延迟
    errors: 'counter',         // 错误数
    rateLimitHits: 'counter'   // 限流次数
  },
  
  // 消息指标
  messages: {
    published: 'counter',      // 发布消息数
    delivered: 'counter',      // 推送消息数
    failed: 'counter',         // 失败消息数
    queueLength: 'gauge',      // 队列长度
    priority: 'histogram'      // 优先级分布
  },
  
  // 连接指标
  connections: {
    active: 'gauge',           // 活跃连接数
    reconnect: 'counter',      // 重连次数
    disconnected: 'counter'    // 断开连接数
  },
  
  // 系统指标
  system: {
    cpu: 'gauge',              // CPU 使用率
    memory: 'gauge',           // 内存使用率
    disk: 'gauge',             // 磁盘使用率
    network: 'gauge'           // 网络带宽
  }
};
```

### 日志策略

```typescript
// 日志级别配置
const LOG_LEVELS = {
  development: 'debug',
  staging: 'info',
  production: 'warn'
};

// 日志结构
interface LogEntry {
  timestamp: string;
  level: 'debug' | 'info' | 'warn' | 'error';
  service: string;
  action: string;
  message: string;
  metadata?: Record<string, unknown>;
  error?: {
    name: string;
    message: string;
    stack?: string;
  };
}

// 审计日志 (独立存储)
const auditLog = {
  retention: '90d',
  storage: 'database',  // PostgreSQL audit_logs 表
  indexing: ['action', 'channelId', 'createdAt']
};
```

### 健康检查

```typescript
// /api/cron/health 端点
async function healthCheck(): Promise<HealthStatus> {
  const checks = {
    database: await checkDatabase(),
    redis: await checkRedis(),
    storage: await checkStorage(),
    queue: await checkMessageQueue()
  };
  
  const isHealthy = Object.values(checks).every(check => check.status === 'healthy');
  
  return {
    status: isHealthy ? 'healthy' : 'degraded',
    timestamp: new Date().toISOString(),
    uptime: process.uptime(),
    checks
  };
}
```

### 告警规则

```yaml
# 告警配置示例
alerts:
  - name: API High Error Rate
    condition: error_rate > 5% for 5 minutes
    severity: warning
    action: page_oncall
    
  - name: SSE Connection Drop
    condition: disconnect_rate > 10% for 1 minute
    severity: warning
    action: notify_slack
    
  - name: Message Queue Backlog
    condition: queue_length > 1000 for 10 minutes
    severity: critical
    action: page_oncall
    
  - name: Database Connection Pool
    condition: connection_usage > 80% for 5 minutes
    severity: warning
    action: notify_slack
```

---

## 总结

### 架构优势

| 优势 | 说明 |
|------|------|
| **模块化** | 分层架构，职责清晰，易于维护 |
| **安全性** | 多层加密，完整的认证授权体系 |
| **可扩展** | 无状态设计，支持水平扩展 |
| **可靠性** | 消息持久化，重试机制，故障转移 |
| **可观测** | 完整的日志、指标、监控 |

### 技术栈总结

| 组件 | 技术选择 |
|------|----------|
| 前端框架 | Next.js 16 + TypeScript |
| 数据库 | PostgreSQL 14+ |
| 缓存 | Redis 7+ |
| ORM | Drizzle ORM |
| 验证 | Zod |
| 测试 | Vitest |
| 部署 | Vercel / Docker |

### 演进路线

```
┌─────────────────────────────────────────────────────────────────┐
│                      技术演进路线                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Phase 1: 基础功能 (当前)                                        │
│  ────────────────────────────                                   │
│  ✓ 端到端加密                                                   │
│  ✓ SSE 实时推送                                                 │
│  ✓ 消息优先级                                                   │
│  ✓ 审计日志                                                     │
│                                                                 │
│  Phase 2: 增强功能 (规划中)                                      │
│  ──────────────────────────                                     │
│  ○ Webhooks 通知                                                 │
│  ○ 消息确认机制                                                 │
│  ○ 消息历史检索                                                 │
│  ○ 多租户支持                                                   │
│                                                                 │
│  Phase 3: 高级功能 (未来)                                        │
│  ──────────────────────                                         │
│  ○ 消息端到端确认                                               │
│  ○ 离线消息推送                                                 │
│  ○ 消息加密密钥轮换                                             │
│  ○ 分布式追踪 (OpenTelemetry)                                   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

<div align="center">

**[🏠 首页](../README.md)** • **[📖 用户指南](USER_GUIDE.md)** • **[📘 API 参考](API_REFERENCE.md)**

</div>
