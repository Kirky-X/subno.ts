...
import type { HttpClient, HttpRequestOptions, HttpResponse } from '../utils/http.js';
import { MockResponses, TestDataFactory } from './config.js';
import { SecureNotifyError, ErrorCode } from '../types/errors.js';
...
export class MockHttpClient implements Pick<HttpClient, keyof HttpClient> {
  ...
  private setupDefaultHandlers(): void {
    ...
    this.onGet('/api/publish', async options => {
      const channel =
        (options.query as Record<string, string> | undefined)?.['channel'] ??
  ...
  async request<T>(options: HttpRequestOptions): Promise<HttpResponse<T>> {
    ...
    if (this.responseOverrides.has(overrideKey)) {
      ...
      if (override.status < 200 || override.status >= 300) {
        const errorData = override.data as {
          success?: boolean;
        ...
        throw new SecureNotifyError(ErrorCode.INTERNAL_ERROR, `HTTP ${override.status}: Error`, {
