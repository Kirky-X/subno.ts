...
describe('Crypto Utils', () => {
  ...
  describe('encrypt/decrypt', () => {
    ...
    it('应该处理被篡改的密文', async () => {
      ...
      await expect(decrypt(tampered as any, password)).rejects.toThrow();
    });
  ...
  describe('hashSHA256', () => {
    it('应该生成固定长度的哈希', () => {
      const hash = hashSHA256('test-token');
    ...
    it('应该处理长字符串', () => {
      ...
      const hash = hashSHA256(longString);
    ...
    it('应该处理 Unicode 字符', () => {
      ...
      const hash = hashSHA256(unicodeString);
