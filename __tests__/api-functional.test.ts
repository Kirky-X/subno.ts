...
describe('API端点功能测试', () => {
  ...
  describe('DELETE /api/keys/:id - 密钥删除', () => {
    describe('两阶段确认删除模式', () => {
      it('应成功确认撤销并删除密钥', async () => {
        ...
        const _mockConfirmation = createMockRevocationConfirmation();
    ...
    describe('管理员直接删除模式', () => {
      ...
      it('应在管理员密钥错误时拒绝访问', async () => {
        ...
        const isValid =
          bufA.length === bufB.length && (await import('crypto')).timingSafeEqual(bufA, bufB);
  ...
  describe('POST /api/keys/:id/revoke - 请求密钥撤销', () => {
    describe('正常流程', () => {
      it('应成功创建撤销请求', async () => {
        const _mockKey = createMockPublicKey();
        const _mockConfirmation = createMockRevocationConfirmation();
  ...
  describe('GET /api/keys/:id/revoke - 查询撤销状态', () => {
    ...
    describe('错误处理', () => {
      ...
      it('应在无待处理撤销时返回NOT_FOUND', async () => {
        ...
        const result =
          await mockKeyRevocationService.getPendingRevocationByKeyId('key_without_revocation');
  ...
  describe('POST /api/keys/:id/revoke/cancel - 取消撤销', () => {
    describe('正常流程', () => {
      it('应成功取消待处理的撤销', async () => {
        ...
        const hasPermission = await mockApiKeyRepository.validatePermission(
          'apikey_test123',
          'key_revoke',
        );
        const result = await mockKeyRevocationService.cancelRevocation(
          'revocation_test123',
          'apikey_test123',
        );
...
describe('边界情况测试', () => {
  ...
  describe('权限边界', () => {
    ...
    it('应正确处理admin权限的全局访问', async () => {
      ...
      const hasAll = ['read', 'write', 'key_revoke', 'admin'].every(p =>
        hierarchy['admin'].includes(p),
      );
...
describe('错误场景测试', () => {
  ...
  describe('认证失败', () => {
    ...
    it('应在API密钥已停用时返回INACTIVE_API_KEY', async () => {
      mockApiKeyRepository.findByKeyHash.mockResolvedValue(createMockApiKey({ isActive: false }));
    ...
    it('应在API密钥已撤销时返回REVOKED_API_KEY', async () => {
      mockApiKeyRepository.findByKeyHash.mockResolvedValue(createMockApiKey({ isDeleted: true }));
...
describe('安全测试', () => {
  ...
  describe('时序攻击防护', () => {
    it('应使用常量时间比较验证管理员密钥', async () => {
      ...
      const _key2 = Buffer.from('test_key_2');
...
describe('集成测试场景', () => {
  ...
  describe('完整撤销流程', () => {
    ...
    it('应在权限不足时阻止完整流程', async () => {
      const userId = 'user_test123';
      const _otherUserId = 'user_other456';
      const keyId = 'key_test123';
      const apiKeyId = 'apikey_test123';
      const channelId = 'channel_test123';
