...
export const DELETE = withErrorHandler(
  async (request: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
    ...
    if (confirmationCode && apiKey) {
      // 使用标准中间件验证 API key（修复 H1：避免原始 key 进入日志/响应）
    ...
    if (adminKey) {
      ...
      // 执行软删除
      const deletedKey = await publicKeyRepository.softDelete(keyId, 'admin_direct', body.reason);
